...
<unattend xmlns="urn:schemas-microsoft-com:unattend">
                ...
                <OSImage>
                    ...
                    <InstallFrom>
                        <MetaData wcm:action="add">
                            <Key>${EditionIndexKey}</Key>
                            <Value>${EditionIndexValue}</Value> </MetaData>
                    </InstallFrom>
                </OSImage>
            </ImageInstall>
            <UserData>
    ...
    <settings pass="oobeSystem">
        <component name="Microsoft-Windows-Shell-Setup" processorArchitecture="amd64" publicKeyToken="31bf3856ad364e35" language="neutral" versionScope="nonSxS" xmlns:wcm="http://schemas.microsoft.com/WMIConfig/2002/State" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
            <UserAccounts>
                <AdministratorPassword><Value>${AdminPassword}</Value><PlainText>true</PlainText></AdministratorPassword>
            </UserAccounts>
            <AutoLogon>
            ...
            <FirstLogonCommands>
                <SynchronousCommand wcm:action="add">
                    <Order>1</Order>
                    <CommandLine>powershell -ExecutionPolicy Bypass -Command "Get-PSDrive -PSProvider FileSystem | ForEach-Object { $path = Join-Path $_.Root 'enable-winrm.ps1'; if (Test-Path $path) { &amp; $path } }"</CommandLine>
