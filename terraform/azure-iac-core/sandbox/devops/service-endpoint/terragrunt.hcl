...
inputs = {
  ...
  azurerm_name = "svc_connection_azure_rm" ## THIS IMPACTS PIPELINES, IF UPDATED THEN YOU NEED TO FIND/REPLACE ON ALL PIPELINE YAMLS
  azurerepos_name = "azurerepos" ## THIS IMPACTS PIPELINES, IF UPDATED THEN YOU NEED TO FIND/REPLACE ON ALL PIPELINE YAMLS
  subscription_id = local.subscription_id 
  authentication_scheme = "ManagedServiceIdentity"
  personal_access_token = local.personal_access_token
  org_name = local.organization
  description_repos = "managed by terraform"
